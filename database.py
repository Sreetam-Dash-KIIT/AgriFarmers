...
def vehicle_registration_page():
    return send_from_directory(BASE_DIR, "vehicleregistration.html")
...
def register_vehicle():
    ...
    try:
        ...
        return jsonify({
            "error": "Driver ID and capacity must be valid numbers"
        }), 400
    ...
    if existing_driver_vehicle:
        ...
        try:
            ...
            if "vehicle_number" in str(e).lower():
                return jsonify({
                    "error": "This vehicle registration number is already registered"
                }), 409

            return jsonify({
                "error": "Could not register vehicle"
            }), 409
